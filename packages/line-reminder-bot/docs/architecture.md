...
| 層 | 責務 |
|---|---|
| index.ts | HTTPリクエスト/レスポンス、Webhook署名検証、イベント解析、ルーティング |
| Controller | イベントデータ変換、ユーザー認証、Usecase呼び出し、結果フォーマット、LINE API呼び出し |
| Usecase | ビジネスロジック、結果オブジェクトを返す、外部APIに依存しない |
| Repository | D1データベースアクセス、データ永続化・取得 |

### index.ts（Router/Handler層）

主にHTTPプロトコルの処理とセキュリティに関わる責務を持ちます。Webhook署名検証やイベント解析など、リクエストの受け取りと初期処理を担当します。
...
- **セキュリティは入口で**: Webhook署名検証はリクエストを受け取った最初の段階で行うべき。不正なリクエストを早期に弾くことで、後続の処理を保護する
...
- **薄く保つ**: ドメイン知識（Vo作成、ビジネスルール）を持たせないことで、プラットフォームの変更（HTTPからgRPCへの移行等）に対して柔軟性を保つ
...
主にビジネスロジックのオーケストレーションとプレゼンテーションの責務を持ちます。イベントデータの変換やユーザー認証、Usecaseの呼び出し、結果のフォーマットなどを担当します。
...
- **ビジネスルールとしての認証**: ユーザー認証はビジネスルール（「誰がこの操作を許可されているか」）であり、プロトコルレベルの検証（Webhook署名）とは異なる
