...
- **判定条件**: メッセージが「削除 」「delete 」「更新 」「update 」で始まる
...
if (isTextMessageEvent(event)) {
  ...
  // 個別削除
  if (message.startsWith('削除 ') || message.toLowerCase().startsWith('delete ')) {
    const id = extractIdFromCommand(message, /^(削除|delete)\s+/i);

    await deleteIndividualReminder({
      id,
  ...
  // 個別更新
  if (message.startsWith('更新 ') || message.toLowerCase().startsWith('update ')) {
    const { id, newMessage } = extractUpdateCommand(message);

    await updateIndividualReminder({
      id,
      newMessage,
      userId: messageEvent.userId,
      replyToken: messageEvent.replyToken,
...
D1 Database
  │
  │ 5. DELETE FROM reminders
  │    WHERE id LIKE 'f47ac10b%'
  │    AND user_id = ?
  ▼
...
db: D1Database
userId: string
id: string  // 完全なUUIDまたは短縮形（最初の8文字など）
```
...
1. 正規表現でコマンド部分を除去
2. 残りを空白で分割
3. 最初の要素が ID、残りが新メッセージ

**例**:
- 入力: `更新 f47ac10b 薬を飲む`
- 出力: `{ id: 'f47ac10b', newMessage: '薬を飲む' }`
...
2. `UPDATE reminders SET execution_time = ? WHERE id = ? AND user_id = ?`
