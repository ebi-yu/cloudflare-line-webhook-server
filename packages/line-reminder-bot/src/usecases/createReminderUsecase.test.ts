...
describe('createReminder ユースケース', () => {
	...
	it('(given) 有効なメッセージとユーザーIDが与えられたとき、 (when) createReminderを実行すると、 (then) DBに5件のリマインダーが保存される', async () => {
		...
		// Assert
		const count = await env.DB.prepare('SELECT COUNT(*) as count FROM reminders WHERE user_id = ?').bind(userId).first<{ count: number }>();
	...
	it('(given) 有効なメッセージとユーザーIDが与えられたとき、 (when) createReminderを実行すると、 (then) 全リマインダーが同じgroupIdを持つ', async () => {
		...
		// Assert
		const reminders = await env.DB.prepare('SELECT group_id FROM reminders WHERE user_id = ?').bind(userId).all();
