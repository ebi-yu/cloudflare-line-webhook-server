 ...
 * LINEのメッセージに返信
 */
export async function sendReplyTextMessage(
	replyToken: string,
	message: string,
	accessToken: string,
): Promise<void> {
 ...
 * LINEのFlexメッセージに返信
 */
export async function sendReplyFlexMessage(
	replyToken: string,
	altText: string,
	...
	if (!response.ok) {
		const errorText = await response.text();
		throw new ServerErrorException(
			`Failed to send reply flex message: ${response.status} - ${errorText}`,
		);
	}
}
...
export async function sendFlexMessage(
	...
	if (!response.ok) {
		const errorText = await response.text();
		throw new ServerErrorException(
			`Failed to send flex message: ${response.status} - ${errorText}`,
		);
	}
}
